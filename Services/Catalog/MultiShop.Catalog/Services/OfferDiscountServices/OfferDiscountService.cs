...
namespace MultiShop.Catalog.Services.OfferDiscountServices
{
    public class OfferDiscountService : IOfferDiscountService
    {
        ...
        {
            var value = _mapper
                            .Map<OfferDiscount>(createOfferDiscountDto);
            await _OfferDiscountCollection.InsertOneAsync(value);
        }

        public async Task DeleteOfferDiscountAsync(string id)
        {
            await _OfferDiscountCollection
               .DeleteOneAsync(x => x.OfferDiscountId == id);
        }

        public async Task<List<ResultOfferDiscountDto>> GetAllOfferDiscountAsync()
        {
            var values = await _OfferDiscountCollection
                .Find(x => true)
                .ToListAsync();
        ...
        {
            var values = await _OfferDiscountCollection  // OfferDiscount Collection sql server da tablo gibi düşünülebilir
                  .Find<OfferDiscount>(x => x.OfferDiscountId == id)
                  .FirstOrDefaultAsync();
        ...
        public async Task UpdateOfferDiscountAsync(UpdateOfferDiscountDto updateOfferDiscountDto)
        {
            ...
            await _OfferDiscountCollection
                .FindOneAndReplaceAsync(x => x.OfferDiscountId == updateOfferDiscountDto.OfferDiscountId, values);
        }
    }
}
